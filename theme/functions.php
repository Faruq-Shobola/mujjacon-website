<?php
/**
 * mujjacon functions and definitions
 *
...
if ( ! defined( 'MUJJACON_VERSION' ) ) {
	// Replace the version number of the theme on each release.
	define( 'MUJJACON_VERSION', '1.0.0' );
}

if ( ! function_exists( 'mujjacon_setup' ) ) :
	 ...
	 */
	function mujjacon_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on mujjacon, use a find and replace
		 * to change 'mujjacon' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'mujjacon', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'mujjacon' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'mujjacon_custom_background_args',
				array(
 ...
 */
function mujjacon_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'mujjacon_content_width', 640 );
}
add_action( 'after_setup_theme', 'mujjacon_content_width', 0 );
 ...
 */
function mujjacon_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'mujjacon' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'mujjacon' ),
...
add_action( 'widgets_init', 'mujjacon_widgets_init' );
 ...
 * @return array
 */
function mujjacon_tinymce_add_class( $settings ) {
	$settings['body_class'] = 'block-editor-block-list__layout';
	return $settings;
}
add_filter( 'tiny_mce_before_init', 'mujjacon_tinymce_add_class' );
